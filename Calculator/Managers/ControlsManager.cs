    ...
    public class ControlsManager : BaseManager
    {
        ...
        private static string GetComma()
        {
            ...
            else
            {
                ...
                ButtonType butonType = GetButtonType(value);
                if (butonType == ButtonType.Number) return GetComma(onScreenValue);
                else if (butonType == ButtonType.Operator) commaValue = "0,";
            }

            return commaValue;
        }

        private static string GetComma(string value)
        {
            if (String.IsNullOrEmpty(value)) return ",";

            ButtonType butonType = GetButtonType(value.Last());
            if (butonType == ButtonType.Number)
            {
                return GetComma(value.RemoveLastChar());
            }
            else if(butonType == ButtonType.Operator)
            {
                return ",";
            }
            else
